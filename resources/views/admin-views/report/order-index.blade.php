    ...
    <div class="content container-fluid">
                        ...
                        <div class="text-capitalize">
                            ...
                            <div class="meida flex-column gap-3">
                                <div >
                                    ...
                                    <a href="#">{{auth('web')->user()->f_name.' '.auth('web')->user()->l_name}}</a>
                                </div>

                                <div class="media-body">
                                    <div class="d-flex align-items-center text-nowrap gap-2">
                                        <div >{{translate('date')}}</div>
                                        <div>( {{ date("d-m-Y", strtotime(session('from_date')))}} - {{date("d-m-Y", strtotime(session('to_date')))}} )</div>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="row">
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $delivered=\App\Models\Order::where(['status'=>4])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                <a href="{{route('admin.order.list',['status'=>4])}}" style="cursor:pointer">
                                    <div class="media">
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $pending=\App\Models\Order::where(['status'=>1])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                <a href="{{route('admin.order.list',['status'=>1])}}" style="cursor:pointer">
                                    <div class="media">
                                        <i class="tio-shopping-cart-off nav-icon"></i>
                                        <div class="media-body">
                                            <h4 class="mb-1">{{translate('pending')}}</h4>
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $failed=\App\Models\Order::where(['status'=>3])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                <a href="{{route('admin.order.list',['status'=>3])}}" style="cursor:pointer">
                                    <div class="media">
                                        <i class="tio-message-failed nav-icon"></i>

                                        <div class="media-body">
                                            <h4 class="mb-1">{{translate('stored')}}</h4>
            ...
            <div class="col-sm-6 col-xl-3 mb-3">
                @php
                    $canceled=\App\Models\Order::where(['status'=>5])->whereBetween('created_at', [$from, $to])->count()
                @endphp
                <div class="card card-sm">
                    <div class="card-body">
                        <div class="row">
                            <div class="col">
                                <a href="{{route('admin.order.list',['status'=>5])}}" style="cursor:pointer">
                                    <div class="media">
        ...
        <div class="row">
            @php
                $services=\App\Models\Category::where('is_sejade', 1)->get()
            @endphp
            @foreach($services as $key=>$service)
                <div class="report-product-item card pos-single-product-card ">
                    <div class="">
                        <div class="report-product-item_thumb">
                            <img src="{{$service['image']}}"
                                     class="img-fit"
                                 alt="{{ translate('product') }}" />
                        </div>
                        <div class="report-product-item_content clickable">
                            <div class="pos-product-item_title">
                                {{ Str::limit($service['name'], 15) }}
                            </div>
                        </div>
                    </div>
                    <div class="report-product-item_price">
                        {{ $service->ordersStatisticsForCurrentYear()['total_amount'] }}
                    </div>
            ...
            @php
                $delivered=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $delivered[$i]=\App\Models\Order::where(['status'=>4])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $ret=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $ret[$i]=\App\Models\Order::where(['status'=>1])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $fai=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $fai[$i]=\App\Models\Order::where(['status'=>3])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
            ...
            @php
                $can=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $can[$i]=\App\Models\Order::where(['status'=>5])->whereBetween('created_at', [$from, $to])->count();
                    }
            @endphp
        ...
        <div class="card">
            ...
            <div class="card-body card-body-height">
                @php
                    $orders= \App\Models\Order::where('status','!=', 5)->whereBetween('created_at', [now()->startOfWeek(), now()->endOfWeek()])->get();
                @endphp
                            ...
                            <a class="card card-border-left border-left-primary shadow-none rounded-0"
                                ...
                                <div class="card-body py-0">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-auto align-self-sm-end">
                                            <div>
                                                {{translate('status')}} <strong> : {{$order['status']}} <br></strong>
                                            </div>
                                        </div>
                                    </div>
                                </div>
