    ...
    <div class="content container-fluid">
        ...
        <div class="row">
            @php
                ...
                $to = session('to_date');
                $totalTax=\App\Models\Expense::where('status', 1)
                    ->whereBetween('created_at', [$from, $to])
                    ->sum('amount');
               if($totalTax==0){
                   $totalTax=0.0;
               }
               $totalSold=\App\Models\Order::where('status','!=', 5)->whereBetween('created_at', [$from, $to])->sum('paid');
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        <h4 class="mb-1">{{translate('total sale')}}</h4>
                                        <span class="font-size-lg text-success">
                                          <i class="tio-trending-up"></i> {{round(abs($totalSold)) }} $
                                        </span>
                                    </div>
                            ...
                            <div class="col-auto">
                                <div class="js-circle"
                                     data-hs-circles-options='{
                                       "value": {{$totalSold=='.01'?0:round(($totalSold/($totalSold+$totalTax))*100)}},
                                       "maxValue": 100,
                                       "duration": 2000,
                                       "isViewportInit": true,
                                       "colors": ["#e7eaf3", "green"],
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="media">
                                    ...
                                    <div class="media-body">
                                        <h4 class="mb-1">{{translate('total expense')}}</h4>
                                        <span class="font-size-lg text-warning">
                                          <i class="tio-trending-up"></i> {{$totalTax}} $
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-auto">
                                <div class="js-circle" data-hs-circles-options='{
                                   "value": {{$totalTax=='0.01'?0:round(($totalTax/($totalTax+$totalSold))*100)}},
                                   "maxValue": 100,
                                   "duration": 2000,
                                   "isViewportInit": true,
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-3">
                @php
                    $yearlySold=\App\Models\Order::where('status','!=', 5)->whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('paid');
                    $yearlexpense=\App\Models\Expense::whereBetween('created_at', [date('y-01-01'), date('y-12-31')])->sum('amount');
                @endphp
                <h6 class="card-subtitle mb-0">{{translate('total sale')}} {{date('Y')}} : 
                    <span class="h3 ml-sm-2"> {{  $yearlySold - $yearlexpense }}</span>
                </h6>
            </div>
            ...
            @php
                ...
                $sold=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $temp =\App\Models\Order::where('status','!=', 5)->whereBetween('created_at', [$from, $to])->sum('paid');
                        if($temp>$maxNumber){
                            $maxNumber=$temp;
                        }
                        $sold[$i]=$temp;
            ...
            @php
                $expense=[];
                    for ($i=1;$i<=12;$i++){
                        $from = date('Y-'.$i.'-01');
                        $to = date('Y-'.$i.'-30');
                        $temp =\App\Models\Expense::whereBetween('created_at', [$from, $to])->sum('amount');
                        if($temp>$maxNumber){
                            $maxNumber=$temp;
                        }
                        $expense[$i]=$temp;
                ...
                <div class="chartjs-custom height-18rem">
                    <canvas class="js-chart"
                        ...
                        "data": {
                          ...
                          {
                            "data": [{{$expense[1]}},{{$expense[2]}},{{$expense[3]}},{{$expense[4]}},{{$expense[5]}},{{$expense[6]}},{{$expense[7]}},{{$expense[8]}},{{$expense[9]}},{{$expense[10]}},{{$expense[11]}},{{$expense[12]}}],
